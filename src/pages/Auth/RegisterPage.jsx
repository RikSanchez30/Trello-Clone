    ...
    <div className="h-screen flex items-center justify-center w-full  text-black">
      <div className=" shadow-lg  rounded-xl  w-full tablet:w-2/3 laptop:w-1/3 mx-9 ">
        <form onSubmit={onRegister} className="flex flex-col gap-4 p-3 w-full h-[600px] bg-white shadow-lg border border-[#97979752] rounded-xl">
          ...
          <input className="h-16 font-semibold rounded-lg text-black border placeholder:text-gray-600 px-2 outline-none text-xl" type="text" placeholder="Username" name="username" />
          <input className="h-16 font-semibold rounded-lg text-black border placeholder:text-gray-600 px-2 outline-none text-xl" type="email" placeholder="Email" name="email" />
          <div className="w-full flex items-center">
            <input className="py-4 font-semibold rounded-l-lg text-black border border-r-0 rounded-l-lg placeholder:text-gray-600 w-5/6 px-2 outline-none text-xl" type={hiddenPassword ? "password" : "text"} placeholder="Password" name="password" />
            <FontAwesomeIcon onClick={() => setHiddenPassword(!hiddenPassword)} className="text-xl rounded-r-lg border border-l-0 cursor-pointer text-black bg-white py-5 w-16 px-2" icon={hiddenPassword ? faEyeSlash : faEye} />
          </div>
          <button className="w-full text-xl border bg-blue-500 h-16 rounded-2xl text-white font-semibold active:scale-95">Continue</button>
          ...
          <Link className="text-xl font-normal text-end self-end bg-blue-500  select-none h-12 w-1/3 flex items-center justify-center rounded-lg  active:scale-95 text-white" to="/login">Or login</Link>
        </form>
      </div>
