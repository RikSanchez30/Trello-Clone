    ...
    <div className="h-screen bg-[#fafbfc] relative z-20 flex items-center justify-center w-full text-black">
      <div className="shadow-lg rounded-xl w-full tablet:w-2/3 laptop:w-1/3 mx-9 border">
        <form onSubmit={onLogin} className="flex flex-col gap-4 p-3 w-full h-[600px] bg-white border rounded-xl">
          ...
          <input
            className="h-16 font-semibold rounded-lg text-black placeholder:text-gray-600 px-2 outline-none text-2xl border"
            type="email"
            placeholder="Email"
            name="email"
            required
          ...
          <div className="w-full flex items-center">
            <input
              className="py-4 font-semibold rounded-l-lg text-black placeholder:text-gray-600 w-5/6 px-2 outline-none text-2xl border border-r-0 rounded-l-lg"
            ...
            <FontAwesomeIcon
              ...
              className="text-2xl rounded-r-lg cursor-pointer text-black border border-l-0 rounded-r-lg bg-white py-5 w-16 px-2"
              icon={hiddenPassword ? faEyeSlash : faEye}
            />
          </div>
          {error && <p className="text-red-500">{error}</p>}
          <button className="w-full text-xl text-white border bg-blue-500 h-16 rounded-xl active:scale-95">
            Continue
          </button>
          ...
          <Link
            className="text-xl font-semibold text-end self-end bg-blue-500 h-12 w-[150px] flex items-center justify-center p-2 rounded-lg active:scale-95 text-white"
            to="/register"
          >
            Or register
          </Link>
